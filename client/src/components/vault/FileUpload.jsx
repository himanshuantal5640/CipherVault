...
import { UploadCloud, FileText, Lock, AlertTriangle } from 'lucide-react';
import { formatBytes } from '../../utils/formatters';
import { MAX_FILE_SIZE_BYTES } from '../../crypto/cryptoConstants';

export default function FileUpload({ onUploadTriggered, isProcessing }) {
  ...
  const handleStartUpload = () => {
    ...
    onUploadTriggered(selectedFile);
    setSelectedFile(null); // Reset picker after triggering
  };
    ...
    <div className="glass-card p-6 rounded-2xl border border-vault-border space-y-4">
      ...
      >
        ...
        <p className="text-sm font-semibold text-white">
          Click or Drag & Drop File to Encrypt & Store
        </p>
        <p className="text-xs text-slate-400 mt-1 font-mono">
          Supports any file up to 100 MB • Zero Plaintext Upload to Node.js
        </p>
      </div>
      ...
      {selectedFile && (
        <div className="p-4 rounded-xl bg-vault-bg border border-vault-cyan/30 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          >
            ...
            <span>{isProcessing ? 'Processing...' : 'Encrypt & Upload to S3'}</span>
          </button>
        </div>
      )}
